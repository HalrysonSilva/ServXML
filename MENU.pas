...
var
  EsperaForm: TFRMESPERA; // Variável para o formulário de espera
  XMLGeracao: string;
  NumNota: string;
  CodNotaAtual: Integer;
  DataAtual: TDateTime;
  ...
  // Variáveis para correção de NCM
  PosInicioNCM, PosFimNCM, PosCursor, I: Integer;
  NCM_Na_Nota: string; // NCM encontrado no XML
  NCM_Status_Valido: Boolean; // Resultado da consulta no banco
  NumItem: Integer;

  // Variáveis para correção de PIX (Bloco 6)
  PosInicioPix, PosFimDetPagPix, PosCardPix: Integer;
  sDetPagPix: string;

const
  NCM_CORRETO_GENERICO = '00000000';
  BLOCO_CARD_PIX = '<card><tpIntegra>2</tpIntegra></card>';

begin
  // 1. INICIALIZAÇÃO DA TELA DE ESPERA
  EsperaForm := TFRMESPERA.Create(Self);
  EsperaForm.Label1.Caption := 'Aguarde... Modificando XMLs em lote.';
  EsperaForm.Show;
  Application.ProcessMessages;
  ...
  try
    ...
    // 2. PREPARAÇÃO DA QUERY DE NCM (Executada dentro do loop)
    with DataModule1.QRYNCM do
    begin
      // A query deve buscar o NCM (CODIGO) e verificar sua vigência.
      // O campo fimVigencia deve ser NULL ou maior que a data atual.
      SQL.Text := 'SELECT CODIGO FROM NFe_NCM WHERE CODIGO = :pNCM AND (fimVigencia IS NULL OR fimVigencia >= CAST(GETDATE() AS DATE))';
    end;

    while not DataModule1.QRYPSQPENDENTE.Eof do
    begin
      // 3. ATUALIZAÇÃO DO STATUS E COLETA DE DADOS
      ...
      EsperaForm.Label1.Caption := Format('Corrigindo Nota %s...', [NumNota]);
      Application.ProcessMessages;
      ...
      begin
        ...
        // ... (Seções 1 a 4: Assinatura, Data/Hora, tpEmis e Contingência) ...
        ...
        // 5. CORREÇÃO NCM: Busca NCMs no XML e valida contra a tabela NFe_NCM
        PosCursor := 1;
        NumItem := 1;
        ...
        begin
          ...
          begin
            ...
            NCM_Na_Nota := Copy(sXMLAjustado, PosInicioNCM, PosFimNCM - PosInicioNCM);
            NCM_Na_Nota := Trim(NCM_Na_Nota);

            // VALIDAÇÃO NO BANCO DE DADOS (QRYNCM)
            NCM_Status_Valido := False;

            if Length(NCM_Na_Nota) = 8 then
            begin
              DataModule1.QRYNCM.Close;
              DataModule1.QRYNCM.ParamByName('pNCM').AsString := NCM_Na_Nota;
              DataModule1.QRYNCM.Open;

              // Se a query retornou 1 registro, o NCM é válido e vigente
              NCM_Status_Valido := not DataModule1.QRYNCM.IsEmpty;
              DataModule1.QRYNCM.Close;
            end;

            // Critério de Correção: NCM não tem 8 dígitos OU falhou na validação do banco
            if (Length(NCM_Na_Nota) <> 8) or (not NCM_Status_Valido) then
            begin
              ...
              logpendentes.Lines.Add(Format('🛠️ Nota %s: NCM do item %d corrigido de "%s" para "%s" (INVÁLIDO/EXTINTO).',
                [NumNota, NumItem, NCM_Na_Nota, NCM_CORRETO_GENERICO]));
            end;
        ...
        // 6. CORREÇÃO PIX MULTI-PAGAMENTO
        PosInicioPix := 1;
    ...
    // 8. FINALIZAÇÃO DA TELA DE ESPERA
    if Assigned(EsperaForm) then
    begin
      EsperaForm.Hide;
      EsperaForm.Free;
