...
uses
  ...
  ACBrPosPrinter, ACBrNFeDANFeESCPOS, ACBrNFeDANFEClass,
  ACBrDFeReport, ACBrDFeDANFeReport, ACBrBase, ACBrDFe,
  ACBrNFe, ShellAPI, XMLIntf, XMLDoc, zlib,System.IOUtils;
...
type
  TfrmACBrNFe = class(TForm)
    ...
    procedure recuperaxml;
    procedure GravarConfiguracao;
    procedure LerConfiguracao;
    procedure CarregarConfiguracao;
    procedure ConfigurarComponente;
    procedure btnCarregarXMLEnviarClick(Sender: TObject);
    function GerarChaveAcesso(const ANumNota, ASerie, AModelo: string): string;
...
uses
  ...
  ACBrDFeConfiguracoes, ACBrDFeSSL, ACBrDFeOpenSSL, ACBrDFeUtil,pcnAuxiliar,
  ACBrNFeNotasFiscais, ACBrNFeConfiguracoes,
  Frm_Status, Frm_SelecionarCertificado, Frm_ConfiguraSerial,CONEXAOBD,MENU,
  TELAESPERA;
...
begin
  ...
  begin
    ...
    Ide.tpAmb     := taproducao;  //Lembre-se de trocar esta variavel quando for para ambiente de producao
...
procedure TfrmACBrNFe.recuperaxml;
...
var
  ...
  CodUF: Integer;
  i: Integer;
  XMLCompletoRecuperado: string;
  CaminhoCompletoArquivo: string;
  NomeArquivo: string;
begin
  // ******************************************************
  // 1. CARREGAR E APLICAR CONFIGURAÇÕES
  // Isso resolve o erro de seção "NFCe_AN_P" não encontrada.
  // ******************************************************
  LerConfiguracao;
  ConfigurarComponente;

  // Acessa o TForm1 (GERAXML) apenas para obter a Chave de Acesso
  ChaveAcesso := Trim(frmmenu.Editchave.Text);

  // 2. VALIDAÇÃO E OBTENÇÃO DE DADOS DO EMITENTE
  CNPJInteressado := Trim(edtEmitCNPJ.Text);
  UFEmissor := Trim(edtEmitUF.Text);

  if (Length(ChaveAcesso) <> 44) or (CNPJInteressado = '') or (UFEmissor = '') then
  begin
    ShowMessage('❌ Erro: Verifique a Chave (44 dígitos), CNPJ e UF do emitente.');
    Exit;
  end;

  // Converte a UF (PA) para o Código (15)
  CodUF := UFtoCUF(UFEmissor);

  // 3. CHAMADA AO SERVIÇO DE DISTRIBUIÇÃO DFe
  MemoResp.Lines.Clear;
  memoRespWS.Lines.Clear;
  MemoDados.Lines.Clear;
  MemoDados.Lines.Add(Format('🔎 Solicitando XML completo para a Chave: %s', [ChaveAcesso]));
  Application.ProcessMessages;

  // Executa a busca: Codigo da UF, CNPJ do interessado, Chave da NFe
  ACBrNFe1.DistribuicaoDFePorChaveNFe(CodUF, CNPJInteressado, ChaveAcesso);

  // 4. EXIBIÇÃO DA RESPOSTA PADRÃO (SOAP)
  ...
  // 5. PROCESSAMENTO E SALVAMENTO DO XML COMPLETO
  with ACBrNFe1.WebServices.DistribuicaoDFe.retDistDFeInt do
  begin
    MemoDados.Lines.Add(Format('Status SEFAZ: %d - %s', [cStat, xMotivo]));

    // cStat 138: Documento Localizado e Disponível para Download
    if cStat = 138 then
    begin
      for i := 0 to docZip.Count - 1 do
      begin
        // Procura pelo schema schprocNFe (Nota Completa)
        if docZip[i].schema = schprocNFe then
        begin
          XMLCompletoRecuperado := docZip[i].XML; // Obtém o conteúdo <nfeProc>

          // Define caminho e nome do arquivo (usando PathPDF como pasta de destino)
          NomeArquivo := ChaveAcesso + '-procNFe.xml';
          CaminhoCompletoArquivo := IncludeTrailingPathDelimiter(edtPathPDF.Text) + NomeArquivo;

          // Salva o conteúdo no disco (Requer System.IOUtils)
          System.IOUtils.TFile.WriteAllText(CaminhoCompletoArquivo, XMLCompletoRecuperado, TEncoding.UTF8);

          MemoDados.Lines.Add(Format('💾 XML salvo em: %s', [CaminhoCompletoArquivo]));
          ShowMessage(Format('✅ XML Completo Recuperado e salvo: %s', [NomeArquivo]));
          Exit;
        end;
      end;
      ShowMessage('⚠️ Documento localizado, mas o XML completo (<procNFe>) não foi retornado.');
    end
    else
    begin
      ShowMessage(Format('❌ Falha na Distribuição DFe: %d - %s', [cStat, xMotivo]));
    end;
  end;
end;
...
var
  Modelo, Serie, Ano, NumeroInicial, NumeroFinal, Justificativa, CNPJ, XMLInu: String;
  UltimoCodNota: Integer;
begin
  ...
  if not(InputQuery('WebServices Inutilização', 'Série', Serie)) then Exit;
  if not(InputQuery('WebServices Inutilização', 'Número Inicial', NumeroInicial)) then Exit;
  if not(InputQuery('WebServices Inutilização', 'Número Final', NumeroFinal)) then Exit;
  if not(InputQuery('WebServices Inutilização', 'Justificativa', Justificativa)) then Exit;

  CNPJ := edtEmitCNPJ.Text;

  // 🔹 Obtém o último CodNota diretamente dentro da procedure
  try
    DataModule1.QRYBUSCARNOTAS.Close;
    DataModule1.QRYBUSCARNOTAS.SQL.Text := 'SELECT COALESCE(MAX(CodNota), 0) AS UltimoCodNota FROM SERVXML';
    DataModule1.QRYBUSCARNOTAS.Open;
    UltimoCodNota := DataModule1.QRYBUSCARNOTAS.FieldByName('UltimoCodNota').AsInteger + 1;
  except
    on E: Exception do
    begin
      ShowMessage('Erro ao buscar último CodNota: ' + E.Message);
      Exit;
    end;
  end;

  // 🔹 Loop para inutilizar cada nota separadamente
  while StrToInt(NumeroInicial) <= StrToInt(NumeroFinal) do
  begin
    try
      // 🔹 Inutiliza a nota fiscal
      ACBrNFe1.WebServices.Inutiliza(CNPJ, Justificativa, StrToInt(Ano), StrToInt(Modelo),
        StrToInt(Serie), StrToInt(NumeroInicial), StrToInt(NumeroInicial));

      XMLInu := ACBrNFe1.WebServices.Inutilizacao.RetornoWS;

      // 🔹 Salva cada inutilização individualmente no banco
      DataModule1.QRYINSERESERVXML.SQL.Text :=
        'INSERT INTO SERVXML (codnota, codemitente, serie, num_mod_doc, numnota, XMLinutilizada, status) ' +
        'VALUES (:codnota, :codemitente, :num_serie, :num_mod_doc, :num_nota_fiscal, :XMLinutilizada, :status)';

      DataModule1.QRYINSERESERVXML.ParamByName('codnota').AsInteger := UltimoCodNota;
      DataModule1.QRYINSERESERVXML.ParamByName('codemitente').AsInteger := Integer(frmmenu.cmbempresa.Items.Objects[frmmenu.cmbempresa.ItemIndex]);
      DataModule1.QRYINSERESERVXML.ParamByName('serie').AsInteger := StrToInt(Serie);
      DataModule1.QRYINSERESERVXML.ParamByName('num_mod_doc').AsInteger := StrToInt(Modelo);
      DataModule1.QRYINSERESERVXML.ParamByName('numnota').AsInteger := StrToInt(NumeroInicial);
      DataModule1.QRYINSERESERVXML.ParamByName('XMLinutilizada').AsString := XMLInu;
      DataModule1.QRYINSERESERVXML.ParamByName('status').AsString := 'M';  // Adicionando o status fixo 'M'

      DataModule1.QRYINSERESERVXML.ExecSQL;

      // 🔹 Incrementa CodNota para a próxima inserção
      UltimoCodNota := UltimoCodNota + 1;

      // 🔹 Exibe mensagem de sucesso
      MemoDados.Lines.Add('Nota Fiscal ' + NumeroInicial + ' inutilizada e salva no banco.');
    except
      on E: Exception do
        ShowMessage('Erro ao inutilizar a nota ' + NumeroInicial + ': ' + E.Message);
    end;

    // 🔹 Passa para a próxima nota
    NumeroInicial := IntToStr(StrToInt(NumeroInicial) + 1);
  end;

  ShowMessage('Processo de inutilização concluído!');
end;
...
begin
  ...
  try
    ...
    Ini.WriteBool(  'Email', 'SSL',     cbEmailSSL.Checked );
     ...
     Ini.WriteString('Monitorar', 'Endereco', Editemail.Text);
...
begin
  ...
  try
    ...
    cbModeloDF.ItemIndex        := Ini.ReadInteger('Geral', 'ModeloDF',         1);
    ...
    cbUF.ItemIndex := cbUF.Items.IndexOf(Ini.ReadString('WebService', 'UF', 'PA'));
    ...
    ACBrPosPrinter1.Device.ParamsString := INI.ReadString('PosPrinter', 'ParamsString', '');
     editemail.Text := Ini.ReadString('Monitorar', 'Endereco', '');
...
begin
  ...
  if cbModeloDF.ItemIndex = 0 then
    // ACBrNFe1.DANFE := ACBrNFeDANFeRL1 // Lógica de DANFE NF-e
  else
  begin
    // Lógica para DANFCE (mantida como referência)
  end;
  ...
  begin
    ...
    // Configura o Modelo do Documento Fiscal (NF-e/NFC-e)
  ...
  begin
    ...
    // APLICA O AMBIENTE: Usa o ItemIndex do RadioGroup (0=Prod, 1=Homol) e soma 1
    // Isso garante que o código 1 (taProducao) ou 2 (taHomologacao) seja aplicado.
...
procedure TfrmACBrNFe.CarregarConfiguracao;
var
  IniFile: String;
  Ini: TIniFile;
begin
  ...
  Ini := TIniFile.Create(IniFile);
  try
    editEmail.Text := Ini.ReadString('Monitorar', 'Email', '');
...
procedure TfrmACBrNFe.REENVIANOTASSEFAZ;
var
  XMLContent: TStringList;
  XMLStream: TStringStream;
  XMLGeracaoAssinada: string;
  XMLCompleto: string;
  RetornoSOAP: string;
  ChaveAcesso: string;
  ...
  xMotivo: string;
  Ok: Boolean;
  EsperaForm: TFRMESPERA; // Adicionado: Variável do Form de Espera
begin
  XMLContent := TStringList.Create;
  XMLStream := nil;

  frmmenu.logpendentes.Lines.Clear;
  // ... (Limpezas de log duplicadas removidas para clareza)
  ...
  // 2. Cria e exibe o Form de Espera
  EsperaForm := TFRMESPERA.Create(Application.MainForm); // Usa MainForm como Owner para o ciclo de vida
  try
    EsperaForm.Position := poScreenCenter;
    EsperaForm.Label1.Caption := 'Iniciando Reenvio SÍNCRONO de XMLs. Por favor, aguarde...';
    EsperaForm.Show;
    Application.ProcessMessages;

    frmmenu.logpendentes.Lines.Add('>>> INICIANDO REENVIO DE NOTAS PENDENTES <<<');
    frmmenu.logpendentes.Lines.Add(Format('Reenviando %d XMLs para a SEFAZ (Envio Síncrono)...',
      [DataModule1.QRYPSQPENDENTE.RecordCount]));
    Application.ProcessMessages;

    // 3. Itera sobre cada registro pendente no BD
    DataModule1.QRYPSQPENDENTE.First;
    while not DataModule1.QRYPSQPENDENTE.Eof do
    begin
      // ADICIONA SEPARADOR VISUAL
      frmmenu.logpendentes.Lines.Add('----------------------------------------------------');

      try
        // 4. CAPTURA DADOS E XML DO REGISTRO ATUAL
        CodNotaLocal := DataModule1.QRYPSQPENDENTE.FieldByName('CodNota').AsInteger;
        ChaveAcesso  := DataModule1.QRYPSQPENDENTE.FieldByName('XMLchave').AsString;
        ...
        EsperaForm.Label1.Caption := Format('Processando Nota %s... (%d de %d)',
          [NumNota, DataModule1.QRYPSQPENDENTE.RecNo, DataModule1.QRYPSQPENDENTE.RecordCount]);
        Application.ProcessMessages;

        frmmenu.logpendentes.Lines.Add(Format('--- PROCESSANDO NOTA: %s | CodNota: %d ---', [NumNota, CodNotaLocal]));
        ACBrNFe1.NotasFiscais.Clear;

        // 5. CARREGA O XML do BD para o componente
        XMLStream := TStringStream.Create(XMLGeracao, TEncoding.UTF8);
        ACBrNFe1.NotasFiscais.LoadFromStream(XMLStream, False);
        FreeAndNil(XMLStream);

        // 6. Reconfigura os dados do Emitente...
        with ACBrNFe1.NotasFiscais.Items[0].NFe do
        begin
          Emit.CNPJCPF := edtEmitCNPJ.Text;
          // ... (Configurações do Emitente omitidas para brevidade)
          Emit.CRT := StrToCRT(Ok, IntToStr(cbTipoEmpresa.ItemIndex + 1));
        end;
        frmmenu.logpendentes.Lines.Add('... XML carregado. Enviando para a SEFAZ.');
        Application.ProcessMessages;

        // 7. Envio SÍNCRONO (ACBr assina e envia)
        ACBrNFe1.Enviar('1', True, True);

        // 8. CAPTURA DE RETORNO E LÓGICA DE MESCLAGEM
        cStat := ACBrNFe1.WebServices.Enviar.cStat;
        xMotivo := ACBrNFe1.WebServices.Enviar.xMotivo;
        RetornoSOAP := ACBrNFe1.WebServices.Enviar.RetornoWS;

        frmmenu.logpendentes.Lines.Add(Format('✅ RESPOSTA DA SEFAZ: cStat %d | %s', [cStat, xMotivo]));
        Application.ProcessMessages;

        if cStat in [100, 150] then // Autorizada ou Denegada
        begin
          XMLGeracaoAssinada := ACBrNFe1.NotasFiscais.Items[0].GerarXML;
          XMLCompleto := frmmenu.MesclarXMLComProtocolo(XMLGeracaoAssinada, RetornoSOAP);
          frmmenu.AtualizarStatusEProtocolo(CodNotaLocal, ChaveAcesso, cStat, xMotivo, XMLCompleto);

          frmmenu.logpendentes.Lines.Add('⭐⭐ SUCESSO: Nota Autorizada! (XML Protocolado salvo)');
        end
        else // Rejeitada ou Falha
        begin
          frmmenu.AtualizarStatusEProtocolo(CodNotaLocal, ChaveAcesso, cStat, xMotivo, RetornoSOAP);
          frmmenu.logpendentes.Lines.Add('❌ REJEIÇÃO: Nota atualizada no Banco de Dados.');
        end;

        // Exibe o XML de Retorno completo no Log principal (MemoResp) e WebBrowser
        LoadXML(RetornoSOAP, frmACBrNFe.WBResposta); // Usando o WBResposta do frmACBrNFe

      except
        on E: Exception do
        begin
          if Assigned(XMLStream) then FreeAndNil(XMLStream);
          frmmenu.logpendentes.Lines.Add(Format('❌ ERRO CRÍTICO: Falha inesperada ao processar nota %s: %s', [NumNota, E.Message]));
          Application.ProcessMessages;
        end;
      end;
    ...
    frmmenu.logpendentes.Lines.Add('>>> REENVIO CONCLUÍDO <<<');
    ShowMessage('✅ Reenvio automático concluído. Verifique o log e a lista de notas pendentes.');

  finally
    // 9. Libera o Form de Espera
...
function TfrmACBrNFe.GerarChaveAcesso(const ANumNota, ASerie, AModelo: string): string;
var
  UFEmissao: String;
  CodUFStr: String;
  CNPJ: String;
  ...
  iNumNota: Integer;
begin
  Result := 'ERRO: Inicialização de Chave falhou.';

  // 1. Coleta e validação dos dados do emitente
  UFEmissao := Trim(edtEmitUF.Text);
  CNPJ := OnlyNumber(edtEmitCNPJ.Text);

  if (Length(CNPJ) <> 14) or (Length(UFEmissao) <> 2) then
  begin
    Result := 'ERRO: CNPJ ou UF do Emitente inválido/vazio. Selecione a empresa.';
    Exit;
  end;

  // 2. Coleta e validação dos dados da nota
  iNumNota := StrToIntDef(ANumNota, 0);
  if (iNumNota <= 0) or (AModelo = '') or (ASerie = '') then
  begin
    Result := 'ERRO: Número da Nota, Série ou Modelo inválido.';
    Exit;
  end;

  try
    // 3. Formatação dos campos
    CodUF := UFtoCUF(UFEmissao);
    if CodUF = 0 then
    begin
      Result := 'ERRO: Código da UF não encontrado pelo ACBr.';
      Exit;
    end;
    CodUFStr := IntToStrZero(CodUF, 2);

    AnoMes := FormatDateTime('yymm', Date);
    ModeloStr := Copy(AModelo + '00', 1, 2);
    SerieStr := IntToStrZero(StrToInt(ASerie), 3);
    NumeroNotaStr := IntToStrZero(iNumNota, 9);
    CodigoNumerico := IntToStrZero(GerarCodigoDFe(iNumNota), 8);

    // 4. Montagem da chave sem DV
    ChaveSemDV := CodUFStr + AnoMes + CNPJ + ModeloStr + SerieStr + NumeroNotaStr + '1' + CodigoNumerico;

    // 5. Cálculo do dígito verificador
    if GerarDigito(cDV, ChaveSemDV) then
      Result := ChaveSemDV + IntToStr(cDV)
    else
      Result := 'ERRO: Falha no cálculo do Dígito Verificador (Módulo 11).';

  except
    on E: Exception do
      Result := 'ERRO inesperado ao gerar chave: ' + E.Message;
  end;
end;
